    ...
    with patch("platform.python_version", return_value=PYTHON_VERSION):
        result = runner.invoke(
            ...
            [
                "generate",
                "--dir",
                "tests/test_cases/dockerfile_aborted/scenario_1",
            ],
        )
        assert result.exit_code == 1
        assert "[ERROR] No 'settings.py' files were found." in result.stdout
...
def test_generate_no_wsgi_asgi_found():
    with patch("platform.python_version", return_value=PYTHON_VERSION):
        ...
        assert result.exit_code == 1
        assert (
            "[ERROR] No 'wsgi.py' or 'asgi.py' files were found."
            in result.stdout
        )
