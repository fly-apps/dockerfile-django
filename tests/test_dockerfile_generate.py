...
from tests.utils import PYTHON_VERSION, copy_dir_to_tmp_path

runner = CliRunner()

PYTHON_NOT_SUPPORTED = "3.7.17"
PYTHON_SUPPORTED = (
    "3.11.4"  # make sure to keep this updated to a supported version
)
    ...
    for test_folder in os.listdir(TEST_CASE_DIR):
        ...
        if os.path.isdir(test_folder_path):
            for specific_dir in os.listdir(test_folder_path):
                specific_dir_path = os.path.join(
                    test_folder_path, specific_dir
                )
...
def test_dockerfile_generation(scenario_dir, tmp_path):
    with patch("platform.python_version", return_value=PYTHON_VERSION):
        ...
        result = runner.invoke(
            cli, ["generate", "--dir", f"{tmp_path}", "--force"]
        )
        ...
        with open(generated_dockerfile_path, "r", encoding="utf-8") as file:
            generated_dockerfile_contents = ARG_MASK.sub(
                r"\1xxx\2", file.read()
            )

        if "TEST_CAPTURE" in os.environ:
            with open(expected_dockerfile_path, "w", encoding="utf-8") as file:
                file.write(generated_dockerfile_contents)

        with open(expected_dockerfile_path, "r", encoding="utf-8") as file:
            expected_dockerfile_contents = ARG_MASK.sub(
                r"\1xxx\2", file.read()
            )
    ...
    with patch("platform.python_version", return_value=PYTHON_NOT_SUPPORTED):
        result = runner.invoke(
            ...
            [
                "generate",
                "--dir",
                "tests/test_cases/dockerfile_aborted/scenario_2",
            ],
        )
        assert result.exit_code == 1
        assert (
            f"[WARNING] It looks like you are using Python "
            f"{PYTHON_NOT_SUPPORTED}" in result.stdout
        )
...
def test_generate_supported_python():
    with patch("platform.python_version", return_value=PYTHON_SUPPORTED):
        ...
        assert result.exit_code == 1
        assert (
            f"[INFO] Python {PYTHON_SUPPORTED} was detected." in result.stdout
        )
